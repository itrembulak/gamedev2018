<?php
        //addscore.php?username=abcdef123&score=24&max_distance=19&distance=55&games_played=10
        ...
        if($row){
            ...
            $sql = 'UPDATE scores SET score=?, max_distance=?, distance=?, games_played=?, last_update=? WHERE id=?';
            $stmt= $dbCon->prepare($sql);
            $stmt->execute([$row['score'],  $row['max_distance'],  $row['distance'],  $row['games_played'], $last_update, $row['id']]);
        }else{
            $sql = 'INSERT INTO scores (score, max_distance, distance, games_played, username, last_update) VALUES (?,?,?,?,?,?)';
            $stmt= $dbCon->prepare($sql);
            $stmt->execute([$new_score, $new_max_distance,  $new_distance, $new_games_played, $new_username, $last_update]);
        }
